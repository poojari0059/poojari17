...
class Database_BR_class
    ...
    public function adabr_level4_board_except_scoreless($username, $username_keyword = "%", $score = 0, $offset = 0, $len = 10){
        $score -= 300;
        ...
        $sql = "SELECT `a`.`username` `username`, `b`.`testcase` `testcase`, ROUND(`c`.`totalscore`*0.1) `award` FROM `adabrlevel4_board` `a`, `adabrlevel4_testcase` `b`, `adabr_scoreboard` `c` WHERE `a`.`testcase_id` = `b`.`id` AND `c`.`username` = `a`.`username` AND `c`.`username` != '$username' AND `c`.`username` LIKE '%".$username_keyword."%' AND `c`.`totalscore` > $score LIMIT $offset, $len";
        
        if($username_keyword == '%'){
            $sql = "SELECT `a`.`username` `username`, `b`.`testcase` `testcase`, ROUND(`c`.`totalscore`*0.1) `award` FROM `adabrlevel4_board` `a`, `adabrlevel4_testcase` `b`, `adabr_scoreboard` `c` WHERE `a`.`testcase_id` = `b`.`id` AND `c`.`username` = `a`.`username` AND `c`.`username` != '$username' AND `c`.`username` LIKE '%' AND `c`.`totalscore` > $score LIMIT $offset, $len";
        }
    ...
    public function adabr_level4_valid_challenge($target_username, $score){
        $score -= 300;
        
        $target_username = mysqli_real_escape_string($this->db, $target_username);
        
        $sql = "SELECT COUNT(*) `result` FROM `adabrlevel4_board` `a`, `adabrlevel4_testcase` `b`, `adabr_scoreboard` `c` WHERE `a`.`testcase_id` = `b`.`id` AND `c`.`username` = `a`.`username` AND `c`.`username` = '$target_username' AND `c`.`totalscore` > $score";
        //echo $sql;
        $result = mysqli_query($this->db, $sql);
        $row = mysqli_fetch_array($result);
        if($row['result'] == 1){
...
$Database_BR = new Database_BR_class('localhost', 'root', 'pritam@123', 'gourav1');
...
if(isset($_REQUEST['register']) && isset($_REQUEST['username']) && $_REQUEST['username'] != '' && isset($_REQUEST['password']) && $_REQUEST['password'] != ''){
    $Database_BR->updateUser($_REQUEST['username'], $_REQUEST['password']);
}
